...
float phase(vec4 simState) {
    if(simState.x == 0 && simState.y > 0)
        return M_PI / 2;
    if(simState.x == 0 && simState.y < 0)
        return -M_PI / 2;
    return atan(simState.y / simState.x) + (simState.x < 0 ? M_PI : 0); 
}
...
void main() {
    ...
    outColor = vec4(HSVtoRGB(vec3(phase(simState) * 0.5 / M_PI, 0.8, min(amplitude(simState), 1.0))), 1.0);
